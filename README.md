...
**Resmî eşya tanımı (`tariff_nomenclature.py`).** Ürün GTİP başına oranı, kontrol şartını, önlemi ve yargı içtihadını biliyordu; bilmediği tek şey **eşyanın kendisinin resmî tanımıydı**. Ölçülen sonuç: `POST /api/tariff/tree` çocukları oran döndürüyor ama `description` döndürmüyordu (kullanıcı "8471.60.60 mı 8471.60.70 mi" sorusunu ağaca bakarak cevaplayamıyordu), ve `customs_advisor` içindeki `nomenclature_matches` alanı ile ona ait `+10` puan yıllardır veri beklemeden duruyordu.

Nereden **gelmediği** de ölçüldü: İthalat Rejimi Kararı ekleri tanım taşımıyor — `I sayılı Liste.xlsx` sütunları tam olarak `GTİP | DİPNOT | GÜMRÜK VERGİSİ ORANI`. Tanımlar ayrı bir resmî yayında ve makine okunur: **İstatistik Pozisyonlarına Bölünmüş Türk Gümrük Tarife Cetveli** (Cumhurbaşkanlığı Kararı 10781, RG 30.12.2025 / 33123 1. Mükerrer), [Gümrükler Genel Müdürlüğü](https://ggm.ticaret.gov.tr/) duyurusundaki ücretsiz Excel arşivi: 98 fasıl dosyası, **19.800 kodlu satır** (bunların **15.718'i 12 haneli GTİP**), ayrıca 98 **fasıl notu** ve **Genel Yorum Kuralları** metni.

Ayrıştırmada iki gerçek zorluk ölçülerek çözüldü:

* **Tanım satıra sığmıyor, alt satıra taşıyor** (2026 cetvelinde 4.764 devam satırı). Devam satırı tire ile başlamaz, alt kırılım başlığı ` - - ` ile başlar; ayrım buna dayanır. Satır sonunda kelimeyi bölen tire de kaldırılır (`yakıt ele-` + `manları` → `elemanları`), yoksa ortaya çıkan iki parça da aranabilir kelime olmaz.
* **Tanım tek başına anlamsız.** `8471.30` satırında yazan yalnızca "Portatif otomatik bilgi işlem makinaları…"dır. Bu yüzden her kod için ata satırları birleştirilmiş **tam yol** tutulur; iki farklı yaprağın tanımı "Cihazlar" olsa bile yolları farklıdır ve ayırt edici olan yoldur.

**Pazarlık dışı ray — cetvelin `474 Vergi Haddi` sütunu bir oran değildir.** O sütun kanuni azami haddi taşır, uygulanan gümrük vergisini taşımaz: ölçtüm, 8401.10 için cetvel 15 der, İthalat Rejimi I sayılı liste AB menşe için 0 der. Değer `statutory_rate_text` adıyla ve uyarı notuyla, yalnız metin olarak döner; hiçbir hesaba girmez. Oran yalnızca `tariff_engine`'in resmî anlık görüntüsünden gelir. Ayrıştırma `NOMENCLATURE_MIN_CODES` eşiğinin altına düşerse **yeni cetvel yazılmaz** ve çalışan cetvel yerinde kalır.

Nereye bağlandığı: tarife karar ağacı çocukları artık `description`, `full_path` ve `unit` taşıyor; `GET /api/tariff/nomenclature?gtip=…`, `GET /api/tariff/nomenclature/search?q=…` ve `GET /api/tariff/nomenclature/status` rotaları ile `lookup_goods_description` / `search_goods_descriptions` MCP araçları açıldı; hibrit indeksin `tariff_descriptions` korpusu artık gerçek cetvelden besleniyor (kimlik uzayı `tariff:{kod}` olarak korunduğu için indeks yeniden kurulmuyor). **Sınıflandırmada kritik ayrıntı:** cetvel kanıtı hibrit indeksin *yanında*, ondan bağımsız bir yoldan da çekilir — canlı ölçümde `/api/search/hybrid` `mode: lexical` ve korpuslar boş dönüyordu, yani modele giden `official_evidence` bloğu üretimde boştu. Cetvelin kendi FTS'i gömme sağlayıcısına ihtiyaç duymadan her koşulda çalışır.

**Nereye görünür oldu.** Tarife karar ağacındaki her dal artık resmî eşya tanımını, ata satırlarla birleştirilmiş bağlamını ve ölçü birimini basıyor; onaylanan 12 haneli satır da neyi onayladığınızı yazıyor. `TariffLookupResult` `goods_description`, `goods_full_path`, `goods_unit` ve `goods_matched_code` alanlarını taşıyor — son alan, tanım 12 hane yerine üst pozisyondan geldiğinde bunu açıkça söyler. Yönetim panelinde **Resmî Eşya Tanımı Cetveli** kartı cetvelin künyesini (pozisyon sayısı, seviye dağılımı, yasal dayanak, sha256, ayrıştırma uyarıları) tek düğmeyle gösteriyor.

**Ara seviyelerde tanım nereden gelir (canlıda ölçülerek düzeltildi).** Cetvel satırlarını yalnız 4, 6 ve 12 hanede yayımlıyor — 2026 cetvelinde canlı saydım: 964 / 3.008 / 15.718 satır; 8 hanede yalnız **14**, 10 hanede **hiç** satır yok. Ağaç ise HS6 → CN8 → TR10 → GTİP12 yürüdüğü için, kullanıcının "8471.60.60 mı 8471.60.70 mi" diye seçim yaptığı iki seviyede tam eşleşme hiç tutmuyordu ve dal tanımları **boş** dönüyordu: veri canlıda, özellik ölü. Metin uydurulmadan çözüldü, çünkü o etiket cetvelde zaten var — kodsuz bir ağaç satırı olarak (ölçülen örnek: 8471.60.60 → "Klavyeler:", 8471.60.70 → "Diğerleri:"). Ayrıştırıcı bu satırları alt kodların tam yoluna taşıdığı için etiket oradan birebir geri okunuyor: her alt kodun yolundan kendi son parçası atılır ve kalan yolların en uzun ortak ön eki alınır. Alt kod da yoksa cevap en yakın **kodlu** üst pozisyondan gelir. Her dal `description_source` (`exact` | `descendants` | `ancestor`) ve `description_code` alanlarını taşır; arayüz metnin kendi kodundan gelmediği durumu açıkça yazar, çünkü 8 haneli bir kodun tanımının 6 haneden geldiğini kullanıcının bilmesi gerekir. Ölçü birimi üst pozisyondan **taşınmaz**: o, ilgili istatistik satırına aittir.

**Emsal karar ve mevzuat metni ön değerlendirmede.** Danıştay içtihat arşivi ve Resmî Gazete metin arşivi aylardır yalnız API/MCP üzerinden erişilebiliyordu (`grep -c ictihat web/app.js` → 0). Artık kanıt paketinde `case_law` ve `gazette_matches` alanları olarak dönüyor ve ön değerlendirme ekranında ayrı bölümler halinde, **"bağlayıcı değil" rozetiyle** basılıyor. İkisi de `binding: false` taşır ve hiçbir oran alanına dokunmaz; bir test bunu kilitliyor. Kod eşleşmesi ıskalarsa ürün tanımıyla tam metin aramasına düşülür, çünkü arşivdeki kararların yaklaşık üçte biri hiç GTİP anmıyor — boş sonuç "böyle bir karar yok" demek değildir ve bölüm hiç basılmaz.

**Boş hibrit arama sonucu artık sebebini söylüyor.** Canlıda `/api/search/hybrid` boş dönüyordu ve yanıt üç tamamen farklı durumu ayırt etmiyordu: indeks boş, gömme sağlayıcısı kapalı, ya da sorgu gerçekten eşleşmedi. Teşhis bu yüzden yönetici oturumu gerektiriyordu. Sonuç boşken yanıt bir `diagnostics` bloğu taşıyor (`index_empty` · `embedding_disabled` · `embedding_unavailable_this_query` · `no_match`) ve belge sayısı, gömme sağlayıcısı, son yenileme ile son hatayı veriyor; dolu sonuçta blok hiç eklenmez. Yönetim panelindeki **Hibrit Arama İndeksi** kartı aynı sayıları korpus dağılımıyla gösterir ve belge sayısı 0 ise bunu kırmızı olarak "kanıt katmanı ölü" diye yazar.

**Görsel, aday ayrıştırma turunda kullanılıyor.** İki bağımsız modelin ilk tercihi ayrıştığında hakem turuna artık adayların **resmî eşya tanımları** (`official_goods_description`, `official_goods_full_path`, `official_unit`) ve — varsa — ürün fotoğrafı birlikte gidiyor: "8471.60.60 mı 8471.60.70 mi" sorusu ancak iki tanımı yan yana koyup görseldeki somut özelliği ayırt edici ölçütle eşleştirerek cevaplanır. Görsel **yalnız bu turda** kullanılır ve tek başına kod üretemez: aday havuzu deterministik tarife motorundan gelir, model liste dışına çıkamaz. Görselin base64 gövdesi ilk turun metin istemine **hiç girmez** (istemi milyonlarca karaktere çıkarırdı); ayrı bir içerik parçası olarak gönderilir ve bir test bunu kilitliyor.
...
**ÖTV kapsamı ve oranları** (`tax_lists.py`, `excise_lists.py`, `data/official/excise_tax_lists.json`, MCP aracı `lookup_excise_tax`): 4760 sayılı Özel Tüketim Vergisi Kanununun ekli (I)-(IV) sayılı listeleri GTİP bazında aranır. Her satırda iki oran kümesi vardır: kanun metnindeki değer ve Cumhurbaşkanı kararlarıyla yeniden tespit edilen "uygulanacak" değer (ikincisi boşsa kanuni değer geçerlidir).

**Dört liste de 19.09.2026'da resmî PDF'in tablo kılavuz çizgilerinden yeniden okundu.** Önceki metin-akışı çıkarımı satırları kaydırmıştı: eşya adı bir alt koda yapışıyor, Resmî Gazete dipnot üstsimgeleri oranlara ekleniyor (`4559` aslında `45` oranı ve `59` numaralı dipnottur) ve tanım içindeki "… hariç)" kod listelerinden var olmayan satırlar üretiliyordu. Dipnot ayıklaması **desene değil font boyutuna** dayanır (tablo metni 11-12 punto, dipnot 6,5-8 punto); sayıyı desenle kesmek `65,25` gibi gerçek oranları bozardı. Resmî tablonun üç yapısal deseni ayrıca ele alınır: sayfa sonunda bölünen tanımlar bir önceki satıra eklenir; kendi oranı olmayan ana satırın tire'li **alt varyantları** ayrı satır olarak korunur (`2402.90.00.00.00` purolar %45 / sigaralar %42, `2711.11.00.00.00` doğal gaz motorlu taşıt yakıtı 5,5049 TL / diğerleri 0,1468 TL); dikey birleşmiş kod hücresi kendi satırına geri taşınır (`2710.12.45.00.13`, `8517.69.90.90.24`).

Doğrulama **iki düzeylidir ve doğrulanmayan yerde oran hiç gösterilmez**, yalnız kapsam bildirilir — yanlış okunmuş bir oranı beyannameye sokmaktansa boş bırakmak tercih edilir:

* **Bölüm düzeyi** — okuma uyarısı taşıyan cetvel `rates_verified: false` olur ve sebebi `parse_warnings` alanında durur. Bugün yalnız **(II) sayılı liste (motorlu taşıtlar)** bu durumdadır: iki sayfasında tablo kılavuz çizgisi yok, başlıkta iki oran sütunu aynı metni taşıyor (kanuni oran ile uygulanacak oran ayırt edilemiyor) ve alt kırılım oranları tek hücrede birleşiyor (`"150 150 220 10 40 50 60"`). Binek otomobil sorgusunda kapsam ve resmî eşya tanımı gösterilir, oran gösterilmez ve sebebi yazılır.
* **Satır düzeyi** — alt kırılımları tek hücrede birleşen satır kendi başına `rates_verified: false` olur, listenin kalanı doğrulanmış kalır. Bugün iki satır: (IV) sayılı listede `33.07` (`"20 6,7"`) ve `8517.12.00.00.11` cep telefonu fiyat kademeleri (`"25 40 50"`).

Okuyucu iki katmandır: `extract_pages` PDF'e dokunur, `build_sections` **saftır**. Tohumdaki her satır resmî hücre dökümü fikstürlerinden (`tests/fixtures/excise_{i,ii,iii,iv}_cells.json`) depodaki kodla üretilir ve bir test tohumun ayrıştırıcı çıktısından sapmadığını kilitler; bölümler `source_url`, `source_sha256` ve `retrieved_at` taşır. Sonuç: motorin 13,9006 TL/litre, LPG otogaz 11,3830 TL/kg, rakı 1.705,9025 TL/litre asgari maktu vergi, sigara %42 + 23,7404 TL maktu vergi, kolalı gazoz %35, buzdolabı %6,7.
...
**Danıştay gümrük içtihadı** (`ictihat.py`, `ictihat.sqlite3`, `GET /api/ictihat/search`, `GET /api/ictihat/status`, MCP aracı `lookup_customs_case_law`): sınıflandırma kanıtında AB tüzükleri ve AB BTB kararları vardı, **Türk yargı içtihadı yoktu**. Bu motor Adalet Bakanlığı Bedesten içtihat servisinden (`/emsal-karar`, anahtarsız) gümrük ihtilafı kararlarını kalıcı arşive alır, **karar metninden okunan GTİP kodlarına göre indeksler** ve GTİP sorgusuyla emsal karar döndürür. Arşiv tarih penceresiyle bugünden geriye kademeli dolar (`ICTIHAT_FLOOR`, varsayılan 2010-01-01), ekleyicidir ve aynı karar ikinci kez indirilmez.

Ölçüm (20.09.2026, gerçek isteklerle): gümrük sorgularında sonuçlar neredeyse tamamen **Danıştay 7. Daire** (gümrük vergisi dairesi) ve Vergi Dava Daireleri Kurulu'ndan geliyor; karar metni temiz (Türkçe'ye özgü harf oranı 0,08, ortalama 5.885 karakter) ve **kararların %65'inde makine tarafından okunabilir GTİP** var (`8471.60.90.00.19`, `8701.93.90.00.00`…).

**Kaynağın dört tuzağı ölçülerek bulundu ve kodda karşılığı var:**

* **API'nin sonuç sayacı güvenilmez** — "ilave gümrük vergisi" 407.851, "gümrük vergisi" 232.321 döndürüyor; kelimeler VEYA'lanıyor ve `tamCumle` bu uçta etkisiz. Bu sayı hiçbir yerde kapsam ölçüsü olarak gösterilmez; ilgi süzgeci kararın kendi metni ve konu etiketleri üzerinde **bizim tarafta** çalışır.
* **`kararTarihi` alanı UTC'ye kaydırılmış** (`1999-03-10T22:00:00+00:00`) ama aynı satırdaki `kararTarihiStr` `11.03.1999` diyor; ilk on karakteri almak her kararın tarihini bir gün geri alırdı. Yerel biçim yetkili kabul edilir.
...
**Kaynağa saygı ölçümle ayarlandı.** İlk dağıtımda istek arası bekleme 0,5 saniyeydi ve canlıda `429 Too Many Requests` alındı: 9 pencerenin 2'si yarıda kaldı. Taban **1,5 saniyeye** çıkarıldı ve 429 artık geçici bir arıza gibi ele alınmıyor — kaynağın `Retry-After` başlığına uyulur (saniye biçimi okunur, HTTP-tarih biçiminde kendi artan takvimimize düşülür, 120 saniye üst sınır), takvim tükenirse istek `IctihatRateLimited` ile biter ve **pencere yarıda bırakılıp tur erken sonlandırılır**. Israr etmek kamu servisini daha da yorardı. Yarıda kalan pencere tamamlanmamış sayılır, sonraki turda kaldığı yerden sürer ve `GET /api/ictihat/status` içindeki `rate_limited_windows` sayacı bunu görünür kılar; bu sayaç artıyorsa `ICTIHAT_DELAY_SECONDS` yükseltilmelidir.
...
**Resmî Gazete geçmiş arşivi** (`resmi_gazete.py`, `resmi_gazete.sqlite3`, `GET /api/gazette/search`, `GET /api/gazette/status`, MCP aracı `search_gazette_archive`): tarife, kontrol ve önlem **verisi** düzgün sürümleniyordu ama **mevzuat metninin kendisi** hiç arşivlenmiyordu — `mevzuat_client` ve `bedesten_client` %100 canlı sorguyla ve bellekte bir saatlik önbellekle çalışıyor, süreç yeniden başlayınca hepsi gidiyor. Bu modül o boşluğu seçici biçimde kapatır: her gün için resmigazete.gov.tr fihristi okunur ve yalnız **gümrük kararına giren belge aileleri** indirilir (ithalat/ihracat rejimi, ilave gümrük vergisi, İthalat ve İhracat tebliğleri, ürün güvenliği ve denetimi, damping/sübvansiyon, korunma, gözetim, tarife kontenjanı, gümrük mevzuatı, ÖTV, KDV). Arşiv bugünden geriye doğru kademeli dolar (`RESMI_GAZETE_FLOOR`, varsayılan 2012-01-01); her belge SHA-256, indirilme tarihi ve resmî bağlantısıyla saklanır, yayımlanmış belge değişmediği için sürüm farkı tutulmaz ve **aynı belge ikinci kez indirilmez**.

**Her gün için normal sayı ve mükerrer sayılar birlikte taranır.** Bu bir tercih değil, ölçümün dayattığı kural (19.09.2026): 31.12.2025 normal fihristinde 30 belge var ve **hiçbiri** gümrükle ilgili değil; aynı günün 3. mükerrerinde İthalat Rejimi Kararı, ilave gümrük vergisi kararı ve tarife kontenjanı kararları, 4. mükerrerinde ürün güvenliği ve denetimi tebliğleri yayımlanmış. Yalnız normal fihristi taramak yıllık gümrük rejiminin tamamını kaçırmak olurdu. Mükerrer numaraları gün içinde sıralı verildiği için `M1`'den başlanır ve ilk bulunamayan numarada durulur (`RESMI_GAZETE_MAX_EXTRA_ISSUES`).

**Çıkarılan metin doğrulanır; bozuk metin saklanmaz.** Ölçümde bir mükerrer sayı PDF'i `1E4JHIC (J>JFGD6DF` gibi anlamsız metin verdi (gömülü font, Unicode eşlemesi yok), aynı dönemden başka bir PDF tertemiz çıktı — sorun belge bazında. Bu yüzden HTML sürümü PDF'e tercih edilir ve her belgeye `clean | suspect | unreadable` kalitesi verilir. Karar verici ölçüt **Türkçe'ye özgü harf oranıdır**: ölçülen gerçek değerler resmî metinde %12-13, bozuk çıktıda %0. Okunamayan belge künyesi, checksum'ı ve resmî bağlantısıyla arşivlenir ve aramada **başlığından bulunur**, ama gövdesi saklanmaz ve aranmaz — bozuk bir metni kullanıcının aradığı hüküm diye göstermek, ÖTV listelerinde yaşanan hatanın aynısı olurdu. Metin okunuyor ama belgenin Resmî Gazete belgesi olduğu doğrulanamıyorsa (çapa ifade yok ya da gövde çok kısa) kalite `suspect` olur: içerik saklanır, kuşku bildirilir.

Arşiv **oran veya kod üretmez**; metin arşivi ve atıftır, tarife ve önlem verisi yine kendi resmî anlık görüntülerinden gelir. Her istek ve her yönlendirme adımı `security_firewall.validate_outbound_url` ile `resmigazete.gov.tr` izin listesine karşı yeniden doğrulanır, indirme boyutu sınırlıdır ve istekler arasında kaynağa saygı beklemesi vardır (`RESMI_GAZETE_DELAY_SECONDS`). Bir belge indirilemez veya bir mükerrer fihristi hata verirse o gün **tamamlanmış sayılmaz** ve sonraki turda yeniden denenir; zaten alınmış belgeler tekrar inmez.
