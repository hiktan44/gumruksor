...
**Hedef ülke KDV oranı** (`eu_vat.py`, `data/official/eu_vat_rates.json`, `GET /api/foreign/vat?iso2=&gtip=`): AB-27 için standart, indirimli, süper indirimli ve park oranları tutulur. AB'nin resmî veritabanı **TEDB** bu tarihte başsız sorguya kapalıdır — arama ucu her istekte 500 dönüyor ve Excel dışa aktarımı sunucu oturumundaki son aramayı verdiği için anonim istekte boş şablon geliyor (canlı olarak ölçüldü). Bu yüzden veri **elle derlenmiş bir tohumdan** gelir ve **hiçbir satır doğrulanmış sayılmaz**; her satır o ülkenin resmî vergi idaresine bağlantı taşır ve son iki yılda oranı değişen ülkeler (`verify_first`) öncelikli doğrulama için işaretlidir. Günlük eşitleme TEDB'yi yoklar; TEDB 27 satırlık gerçek veri döndürdüğü gün tohum kendiliğinden resmî veriyle değişir — eksik veya boş yanıt tohumu **asla** ezmez.
