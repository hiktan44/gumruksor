...
**ÖTV kapsamı ve oranları** (`tax_lists.py`, `data/official/excise_tax_lists.json`, MCP aracı `lookup_excise_tax`): 4760 sayılı Özel Tüketim Vergisi Kanununun ekli (I)-(IV) sayılı listeleri GTİP bazında aranır. Her satırda iki oran kümesi vardır: kanun metnindeki değer ve Cumhurbaşkanı kararlarıyla yeniden tespit edilen "uygulanacak" değer (ikincisi boşsa kanuni değer geçerlidir). Bölüm bazında bir `rates_verified` bayrağı taşınır ve **doğrulanmamış bölümde oran hiç gösterilmez**, yalnız kapsam bildirilir — yanlış okunmuş bir oranı beyannameye sokmaktansa boş bırakmak tercih edilir.

**(III) sayılı liste** (alkollü içecekler, tütün mamulleri, kolalı gazozlar) 19.09.2026'da `excise_lists.py` ile resmî PDF'in **tablo kılavuz çizgilerinden** yeniden okundu ve doğrulandı. Önceki metin-akışı çıkarımında satırlar kaymıştı (eşya adı bir alt koda yapışıyordu) ve Resmî Gazete dipnot üstsimgeleri oranlara ekleniyordu — `4559` aslında `45` oranı ve `59` numaralı dipnottur. Ayıklama **desene değil font boyutuna** dayanır (tablo metni 11-12 punto, dipnot 6,5-8 punto); sayıyı desenle kesmek `65,25` gibi gerçek oranları bozardı. Ayrıca (B) cetvelindeki `2402.90.00.00.00` gibi ana satırların kendi oranı yoktur, altındaki tire'li alt varyantlar farklı oran taşır (purolar %45, sigaralar %42); bunlar ayrı satır olarak korunur. Sonuç: rakı, bira, şarap, sigara ve kolalı gazoz sorgularında artık oran ve asgari maktu vergi tutarı görünür. Okuyucu iki katmandır — `extract_pages` PDF'e dokunur, `build_sections` saftır — ve tohumdaki her satır resmî hücre dökümü fikstüründen (`tests/fixtures/excise_iii_cells.json`) depodaki kodla üretilir; test bunu kilitler. (I), (II) ve (IV) sayılı listeler metin akışından çıkarılmış hâliyle korunmaktadır: o sayfalarda kılavuz çizgisi tespiti her sayfada çalışmıyor ((II) sayılı listede iki sayfada tablo bulunamıyor) ve yarım bir yeniden okuma mevcut veriyi bozardı.
